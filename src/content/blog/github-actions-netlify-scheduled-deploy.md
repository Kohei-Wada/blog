...
1. Netlifyのダッシュボードで、対象のサイトを選択します。
2. 「Site settings」→「Build & deploy」→「Build hooks」へ移動します。
3. 「Add build hook」ボタンをクリックします。
4. 任意の名前（例: daily-rebuild）を入力し、デプロイしたいブランチ（通常はmain）を選択して「Save」します。

生成されたURLをコピーしておきます。このURLは外部に漏れないように注意してください
...
4. Nameに Netlify_BUILD_HOOK_URL を、Secretにステップ1でコピーしたビルドフックURLを貼り付けて「Add secret」します。
...
この設定をしてからは手動でのデプロイ作業がなくなり、コードを書いてGitHubにプッシュするだけで、翌日にはブログのポートフォリオも自動的に最新状態になるという理想的な運用が実現できています。
